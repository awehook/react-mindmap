...
const newOptions = {
    ADD_NOTE_RELATION: (props) => {
        ...
        if (!model.getIn(['extData', 'evernote'])) {
            newModel = model.setIn(['extData', 'evernote'], {});
        }
        newModel = newModel.updateIn(['extData', 'evernote'], m => { m[topicKey] = note; return m; });
        return newModel;
    }, 
...
function HotKeyPlugin() {
    return {
      ...
      renderTopicContentOthers: function(ctx, next) {
          ...
          const evernoteData = model.getIn(['extData', 'evernote'], {});
          const res = next();
          return <>
              { res }
              { evernoteData.hasOwnProperty(topicKey) && <img key="image1" 
